...
def _normalize_topic_category(title: str, raw_category: str, platform: str) -> str:
    title_text = (title or "").lower()
    category_text = (raw_category or "").lower()
    platform_text = (platform or "").lower()

    for normalized, hints in RAW_CATEGORY_HINTS.items():
        if any(hint.lower() in category_text for hint in hints):
            return NORMALIZED_CATEGORY_LABELS[normalized]

    for normalized, hints in TITLE_CATEGORY_KEYWORDS.items():
        if any(hint.lower() in title_text for hint in hints):
            return NORMALIZED_CATEGORY_LABELS[normalized]

    if platform_text in {"github", "hackernews"}:
        return NORMALIZED_CATEGORY_LABELS["tech"]
    if platform_text == "bilibili":
        return NORMALIZED_CATEGORY_LABELS["anime"]

    return NORMALIZED_CATEGORY_LABELS["other"]
        ...
        for topic in topics or []:
            ...
            item = {
                **topic,
                ...
                "normalized_category": _normalize_topic_category(
                    topic.get("title", ""),
                    topic.get("category", ""),
                    platform,
                ),
        ...
        aggregated_topics.append(
            {
                ...
                "normalized_category": representative.get("normalized_category", "")
                or _normalize_topic_category(
                    representative.get("title", ""),
                    representative.get("category", ""),
                    representative.get("platform", ""),
                ),
