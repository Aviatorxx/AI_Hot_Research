...
export function renderMinePageMarkup(options: {
  ...
  html += `<div class="my-focus-card" id="myDiscoverSection">
  ...
  html += `<div id="keywordSubscriptionSection">
      <div class="my-section-title">🔖 关键词订阅</div>
  ...
  html += `<div id="platformVisibilitySection">
      <div class="my-section-title">🧭 平台展示</div>
      <div class="platform-visibility-copy">选择需要展示的平台，未勾选的平台将从顶部 tabs 和“全部平台”聚合页中隐藏。</div>
  ...
  html += `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0 8px">
      <span style="font-size:12px;color:var(--text-muted);font-family:var(--font-mono)">
        ${likes.length > 0 ? `❤️ ${likes.length} 条收藏` : "❤️ 暂无收藏"}
      </span>
      <button class="btn" style="padding:4px 12px;font-size:11px;color:var(--neon-magenta);border-color:var(--neon-magenta);opacity:${likes.length > 0 ? 1 : 0.4}"
        data-action="showLikesModal" ${likes.length === 0 ? "disabled" : ""}>
        查看收藏
      </button>
    </div>`;

  if (hasCurrentUser) {
    html += `<div id="chatHistorySection">
      <div class="my-section-title" style="margin-top:8px">💬 历史会话</div>
...
export function renderRecommendationsPanelMarkup(options: {
  ...
  const { data, prefixHtml = "", hasTerms, platformNames, escapeHtml, escapeAttr } = options;

  if (!data) {
    return `${prefixHtml}<div class="ai-placeholder">
      ...
      <div class="ai-quick-actions">
        <button class="ai-quick-btn" data-action="fetchRecommendations">刷新推荐</button>
        <button class="ai-quick-btn" data-action="${hasTerms ? "openRelatedHotSection" : "focusKeywordInput"}">${hasTerms ? "查看热榜相关" : "去添加关注"}</button>
        <button class="ai-quick-btn" data-action="openFeedMode" data-mode="resonance">看跨平台共振</button>
        <button class="ai-quick-btn" data-action="openFeedMode" data-mode="rising">看上升最快</button>
      </div>
    </div>`;
  }
  ...
  if (data.query_summary) {
    html += `<div class="ai-overview" style="margin-bottom:12px">
  ...
  if (data.interest_tags && data.interest_tags.length > 0) {
    html += `<div style="margin-bottom:12px">
      <div class="my-section-title">兴趣标签</div>
  ...
  if (data.recommended_topics && data.recommended_topics.length > 0) {
    html += `<div class="my-section-title">为你推荐（${data.recommended_topics.length}条）</div>
    ...
    for (const item of data.recommended_topics) {
      html += `<div class="recommend-topic-item">
        ...
        <div class="recommend-topic-actions">
          ...
          <button class="recommend-topic-action" data-action="addRecommendedKeyword" data-title="${escapeAttr(item.title)}" data-reason="${escapeAttr(item.reason || "")}">加入关注</button>
          <button class="recommend-topic-action" data-action="analyzeTopic" data-title="${escapeAttr(item.title)}">立即分析</button>
        </div>
      </div>`;
    }
    html += "</div>";
  ...
  html += `<div class="ai-quick-actions">
    <button class="ai-quick-btn" data-action="openRelatedHotSection">查看热榜相关</button>
    <button class="ai-quick-btn" data-action="openFeedMode" data-mode="resonance">共振优先</button>
    <button class="ai-quick-btn" data-action="openFeedMode" data-mode="rising">上升最快</button>
    <button class="ai-quick-btn" data-action="fetchRecommendations">刷新推荐</button>
  </div>`;
